  ...
  for (let i = 0; i < project.toDoList.length; i++) {
    if (project.toDoList[i].date) {
      let newToDo = new ToDo(
        project.toDoList[i].name,
        project.toDoList[i].date,
        project.toDoList[i].notes,
...
const remakeUmbrella = (emptyObject) => {
  const general = new Project(
    emptyObject.general.name,
    remakeToDos(emptyObject.general)
  );
  let projectArray = [];
